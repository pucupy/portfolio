# Alejandro Velazquez — portfolio content

Full text of every page, exported 2026-08-06.
...
I design products where the interface is only part of the challenge. I understand the regulations, systems, operations and data behind the product, then turn that complexity into something people can easily use.
I joined Yapily in 2020 as its first designer and built the design function: three design systems, a UX research practice, and the governance behind both. I lead design across the platform and manage two designers. I'm still hands-on.
FOUNDING DESIGNER
Built Yapily's design function from the first hire.
CONTINUOUS RESEARCH
200+ customer interviews since 2020, plus end-user testing.
REGULATED PRODUCTS
Open banking, payments, compliance, enterprise SaaS.
HANDS-ON
Strategy, research, interaction design, systems, prototypes, code.
2020 — PRESENT
...
01
#### Yapily Hosted Pages
2022 — present · Live in beta; optimisation ongoing
...
—End-to-end conversion grew from roughly 20% in the early beta to approximately 65&ndash;75%, varying by use case and market. A platform-wide gain, not a design result — see the case study.
—Bank discovery improved through popular banks, bank pre-selection for returning users, grouping, aliases and market-specific naming.
View case study →
[image: Hosted Pages — bank selection, redesigned · 2200×920px]
02
#### Yapily Console
2020 — present · Configurator shipped; new Console in beta
...
View case study →
[image: Yapily website in 2020, headline reading Open Banking API connectivity beside a yellow block]
2020 — 2021
[image: Yapily website in 2023, headline reading Power your product innovation with open banking]
2023 — 2024
[image: Yapily website in 2025, headline reading Open banking platform for frictionless finance]
2025 — 2026
04
#### HeadBox
2018 — 2019
...
View case study →
[image: The HeadBox Lead Feed, showing event enquiries with location, budget, guest count, timings and an I'm available button on each row]
### Other experience
#### Su4erheroes
2016 — 2018
Co-founded a product design consultancy with former Payfriendz colleagues, working across payments, personal health budgets and marketplace products. The principal client was Docomo Digital, where I designed payment products supporting marketplace and sharing-economy platforms across Europe and Asia.
View case study →
#### Payfriendz
2013 — 2016
...
#### Globant
2008 — 2011 · Argentina
Design Analyst at Globant, a leading IT consultancy from Argentina. Enterprise digital products for clients including Google, JP Morgan, PwC, Coca-Cola and Telefónica, with distributed product and engineering teams.
### Creative archive
Illustration, identity and animation made alongside the product work, much of it drawn by hand, for fun, in the pre-AI era. Including Nerdtees, a 2018 side project honouring scientists who don’t get put on t-shirts, and Minimeter, a baby growth tracker that reached pilot.
View the archive →
### Let's chat
Feel free to contact me if you have any questions or just want to chat about your project/product.
Email Ale
aleuxuk@gmail.com
·
Download CV (PDF)
...
### What I do
I lead design across Yapily’s open banking platform. Hosted Pages is the fastest route onto it: a pre-built consent and bank-connection flow, white-labelled and compliant out of the box, that customers embed in their own product rather than building one bank at a time. Then the console they use to configure and manage their integration, and the design systems underneath both.
...
I work closely with Product, Engineering, Compliance, Customer Success, Support, Sales, Pre-sales, Implementation and senior leadership, and I have managed relationships with branding agencies, research providers and customer design teams.
### What I bring
...
_Source: `hosted-pages.dc.html`_
...
### AT A GLANCE
ROLE
Product strategy, research and design lead
PERIOD
2022 to present
WORKING WITH
Product, frontend, backend, compliance, implementation
STATUS
Live in beta; optimisation ongoing
OUTCOME
Integration from four to six months down to around two weeks · bank discovery from 5–15 seconds to 1–3 in most markets
...
A clickable end-to-end prototype in two viewpoints. On a phone the bank's app opens in place. On a desktop the flow hands off to a phone with a QR code and waits for it to come back.
Open the user flows ↗
### Problem
...
2
Tap the link
Opens the Commerzbank app, or its mobile site
3
Approve the payment
We’ll pick up here
App or mobile web
...
Five branching decisions. Green marks what the customer's integration decides; grey marks what the product, market and device decide.
...
[image: Bank selection after the redesign: a most-popular shortcut above a list of parent brands, each grouping its branches]
AFTER
A most-popular shortcut above brand groups, each holding its branches
...
#### Returning users, and skipping the screen entirely
...
Working with product and with customers directly, we scoped what could reasonably be reused: pre-filling account details such as IBANs where the customer already held them, and skipping bank selection entirely where we already know which bank the person used last time.
The first instinct was to decorate the list: promote the previously used bank into the top slot. Assembled, it read as two optimisations competing for the same space. A returning user does not need a better list; they need no list.
So the two visits get different screens. First-time visitors see one Popular banks block above the full A–Z. Returning users skip selection altogether: the journey opens on the payment screen with their bank already selected, and a Change bank action beside the bank name drops them back into the full list.
The harder problem was that the same screen has to serve customers who want opposite things from it. A customer running a KYC journey wants the bank locked, because the point is verifying one specific account and letting the user wander is a failure. An ecommerce merchant wants the reverse — pre-select their bank, but let them get back to the full list. Both are correct for their use case, so neither can be the design; it has to be configuration.
...
700+ local banks
Deutsche Bank AG
The bank used last time is pre-selected on the payment screen.
Selection is skipped, not re-ordered.
Change bank, beside the bank name, returns to the full list.
Returning users skip the screen rather than getting a re-ordered version of it. The list stays one thing, and the repeat visit costs one tap.
Both states are in the flow above: the first-visit Popular banks block, and the returning-user payment screen with the bank pre-selected — see bank selection ↗.
...
[image: Embedded login after the redesign: the bank named at the top, the regulated entity stated, and the credential fields labelled as the bank labels them]
AFTER
The bank named, the regulated entity stated, the fields labelled as the bank labels them
In context in the flow above — see the embedded login ↗.
#### How this was tested
...
### For the customers Hosted Pages didn’t fit
Enterprise and direct-licence customers sometimes needed more control than a hosted flow allows, so they built their own checkout, consent, bank selection and authentication screens. Rather than leave that to chance, I designed the UX guidance and the interactive demos that showed how those journeys should behave, working with pre-sales and implementation. My team expanded them afterwards.
The demos also solved a sales problem. Open banking infrastructure is invisible: nobody can see a payment initiation. Walldo Wallet and Rocket Shop are fictional products that make one concrete, and the Apple ID and Apple Wallet journeys show the same thing inside an interface people already know. One asset used by sales, pre-sales, implementation and customers.
[image: The Yapily demo hub: Walldo Wallet and Rocket Shop use-case cards, each with a QR code for the mobile demo and links to the developer guides]
The demo hub. Each use case has a mobile demo behind a QR code and a developer guide for the AIS and PIS flows.
[image: Two demo cards, Apple ID and Apple Wallet, each with a QR code for the mobile version and a link to the desktop customer journey]
Adding pay-by-bank to Apple ID, and a bank to Apple Wallet. Both built on Yapily’s API to show what the platform makes possible.
### Reflection
...
2020 to present
WORKING WITH
Product, engineering, support, customer success, implementation
STATUS
Hosted Pages configurator shipped; new Console in beta
OUTCOME
Assisted onboarding for enterprise, self-service for everyone else · configuration moved from support tickets to the customer
...
Rebuilt as live markup from the Yapily UI components, with a switcher between signing up, a non-customer’s sandbox and a customer in production.
01 · GETTING IN
...
[image: Sign in with four authentication routes, account creation, profile completion, organisation setup, the first-run home with three routes out of the empty state, and the organisation switcher]
Four authentication routes rather than one, everything not needed to create an account moved out of it, the organisation named up front because it appears on invoices later, and a first run that offers three ways forward instead of an empty dashboard.
02 · START TESTING
...
[image: Sandbox applications, the application overview with gated metrics, credentials labelled by sensitivity, read-only sandbox scopes, the Hosted Pages configurator, institution filters, webhook delivery logs and an explained empty analytics state]
The sandbox application exists before the customer asks for it, and production sits alongside as a marked upgrade. Credentials are labelled by sensitivity rather than by name; metrics are visibly gated rather than invisibly absent; bank capability is a filter rather than a question for a human. The Hosted Pages configurator is the one piece that shipped in full.
...
[image: Step one of seven asking for products, markets and the licence question, KYB not started and in progress with four named states, sandbox billing, the invite dialog with roles described by capability, and the team screen with a single owner]
The licence question is asked before anything is configured, two expensive retrofits are the reason it comes first. KYB shows four named states rather than a black box, so a customer can see what is blocking them without asking. Roles are described by what they can do, and billing is visible while still free.
04 · OPERATING IT
...
[image: Production home with consent volumes and a persistent quickstart, applications filterable by environment, the application overview with live metrics, scopes and countries, the conversion funnel with a drop-off alert, bank performance ranked by conversion, recent sessions with request IDs, and production webhooks]
The same product with the gates removed rather than a different screen. The funnel flags its own drop-off and attaches a suggestion; bank performance runs from the low nineties to the low seventies, a range design did not cause and cannot close, which is exactly why the customer should see it; sessions carry request IDs so a developer debugs a specific failure rather than inferring it from a rate.
[image: Sub-applications each carrying their own KYB state, team roles, production billing with invoices, verified KYB with its renewal date, SAML SSO configuration and account security settings]
Sub-applications are one construct for two problems — a reseller’s merchants and a customer’s regional entities, each carrying its own compliance state, so one can be live in Germany while its sibling is under review in France. Verification names its renewal date because re-verification is a recurring obligation. Anything a CSM used to export by hand is a queue with a person in it.
### Where it stands
...
BEFORE
A welcome screen with no next step
[image: After — quickstart guide, and password rules stated before you type · 2200×1120px]
AFTER
Quickstart guide, and password rules stated before you type
...
[image: The Console Get Started page — three tabs of guidance ending in a contact form that a Yapily salesperson responds to]
...
# 03 — Design systems at Yapily

_Source: `design-systems.dc.html`_
...
### AT A GLANCE
ROLE
Design system strategy, architecture and governance
PERIOD
2020 to present
WORKING WITH
Design, frontend
...
Shipped and in use
OUTCOME
Around 38 engineers and 4 designers building against them · accessibility encoded in the token layer · governance built jointly with the frontend lead
...
01
Primitives — the raw values. Sixteen colours, one type scale, one spacing base. Nothing in this tier knows what it is for.
02
Semantic aliases — what a primitive means in context: surface, border, text-muted, action, destructive. This is the tier components reference.
03
System-specific — the values only one system needs. Custard’s themable brand slots, Mark’s gradients, Yapily UI’s dense-data greys.
...
[image: The yapily.com homepage rebuilt from Mark — dark teal ground, a gradient heading reading Open banking platform for frictionless finance, a lime primary button and a customer logo row]
...
# 04 — Research practice at Yapily

_Source: `research-practice.dc.html`_
...
[image: A meeting room of Yapily colleagues watching a colleague place sticky notes on a whiteboard, laptops and notebooks open around a long table]
A problem-framing workshop with PMs, engineering and wider teams
[image: Seven Yapily colleagues around a table writing on paper, a laptop showing the instruction: make a list of all the content you think should be there to help people do their job]
A content workshop, the task on screen: list the content you think should be there to help people do their job
...
[image: A Slack message from Simon in Yapily Support: I think you are doing a cracking job here — I like the games, the customer interviews, the write-ups, and the insights you are gaining]
Support, in the channel where the write-ups were posted
← Design systems
HeadBox →
aleuxuk@gmail.com
·
...
# 05 — Yapily brand & website

_Source: `brand-website.dc.html`_
...
Yapily changed how it explained itself five times between 2020 and 2026. Each change needed more than a new headline: the website had to say what customers could buy, how the products fitted together, and why Yapily was not the company next to it in the search results.
Role: Creative direction and product design leadership · Period: 2020 — present
...
[image: Yapily website in 2023: dark green hero reading Power your product innovation with open banking above a mint green statistics band]
...
—An API-first and deliberately invisible infrastructure provider.
—A company with clearer products around the API.
...
### Reflection
The hardest part was maintaining coherence while the company's positioning kept moving. A website cannot solve unclear strategy, but it exposes it. The strongest work came from using the site architecture and product pages to force clearer decisions about what Yapily offered and how the parts related.
← Console
·
HeadBox →
← All work
...
Outcome: £150,228 ARR from 117 customers in three months, four times the prior model · 1,500+ leads in the first month · hosts logging in daily from launch
...
Five engagements, all of them about money moving between people who don’t trust each other yet.
The entries below are the work, not the company.
2016–2018 · LONDON · DESIGN LEAD, DIRECTOR FROM 2018
[image: The Su4erheroes site hero: the S4 wordmark above the line We are SU4ERHEROES, humanity's superfriendly BFFs, with four illustrated superheroes below]
The studio site. Four founders, drawn as the joke the name was.
### NOMO
...
The seller app answered something the web dashboard structurally couldn’t: a seller sells on more than one marketplace, and nobody was giving them a single view of what they’d earned. Payments showed a status timeline rather than a status word: pending on the 13th, paid out on the 15th. Each currency’s account fields used its own format: sort code for sterling, IBAN for euro, routing number for dollar.
...
[image: Three NOMO seller app screens: activity across marketplaces, a payment status timeline, and bank accounts by currency]
The seller app — earnings across every marketplace, payments as a timeline
[image: The NOMO marketing site features page, explaining payouts, escrow and compliance]
The marketing site — explaining an intermediary licence to people who don’t want one
[image: The Huawei promo cashback screen, listing two payments: one paid, one on its way]
The Huawei cashback app — cashback as a payment with a status, not a balance
### Keymono
...
It reached pilot.
[image: Two Keymono screens: choosing between Open, Split and Fixed collection types, and the contributor landing page asking for help with a dinner]
Open, Split and Fixed, each with the situation it fits — and what a contributor sees from the link
[image: Two Keymono screens: a list of collections, and a completed collection with a share link]
Collections in progress, and a completed one with its share link
### Colly
2016–2018 · our own product
...
It reached pilot.
[image: Two Colly screens: a transaction list with the balance in Singapore dollars and the bitcoin figure underneath, and a contact page showing total sent, total received and number of payments]
Your own currency first, the BTC figure underneath — and a contact page rather than a log
[image: Two Colly screens: a withdrawal with the network fee above the button, and a receive screen with QR code]
The fee above the button, not after it
### Caremoni
2017–2018
...
I interviewed every party in the chain and mapped the flows: the two operators, independent carers, care agencies, and the people receiving care. Rather than build first, I ran a concierge version to test whether those flows were right: a plain HTML front end with payments still processed by hand.
...
[image: A service map of the personal health budget payment chain before automation, showing money flowing out on quarterly and monthly cycles and evidence travelling back weekly, ending in a manual audit sent by email]
Before: four clocks, one reconciliation, done by hand
[image: Three Caremoni app screens: authorising a payment, a confidential service rating, and carer registration]
The carer app — authorising a payment, rating a service
[image: The Caremoni admin table of carers, their pay rates and payment status]
Admin — pay rates and payment status in one table
### Mummybay
...
[image: The Mummybay marketplace homepage, a grid of preloved items for children]
The marketplace, a working store, stood up to test the idea
← All work
Creative archive →
aleuxuk@gmail.com
·
...
_Source: `creative-archive.dc.html`_
...
[image: Payfriendz app screens — request, send and card]
App screens, requesting, sending, card
### Illustration and marks
drawn over several years
Portraits, posters and marks drawn for colleagues, friends and side projects. The same reduced vector language I was using for product illustration at the time, pushed to see how much likeness survives a small number of shapes.
[image: Four colleagues drawn as superheroes]
A team, drawn as superheroes
[image: Collection of vector character portraits]
Vector character portraits
[image: Line portrait of Steve Jobs on a repeating apple pattern]
Steve Jobs, on a wall of apples
[image: Split portrait poster reading NOPE]
NOPE, an election poster
[image: Hatched ink portrait of a guitarist]
Guitarist, hatched in ink
[image: Blue creature chasing insects across a yellow field]
A blue thing, chasing bugs
[image: Astronaut sitting on a Mars bar]
Astronaut, resting on Mars
[image: Die-cut stickers of a meditating character on a desk]
Meditating figure, cut as stickers
