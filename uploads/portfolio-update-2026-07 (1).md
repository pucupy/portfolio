...
*Sections 1–4 are content to build. Section 5 is corrections to apply to pages that already exist. Section 6 is open items and should not be built.*
...
Between 2023 and 2026 the programme worked through hundreds of design variants across bank selection, the QR code screen, embedded logins and decoupled flows — layout, copy, iconography and in-flow guidance. Sustained testing at that volume is what makes a flow this constrained improvable at all: there is no single redesign available when the screens are this small and the compliance requirements this fixed, only a long run of small, evidenced changes.
...
## 3. New entry — UX guidance for teams building on open banking
...
Customers operating under their own licence build their own experience rather than using ours. That makes the quality of those flows a business problem as much as a design one: a poor flow loses the connection, and the loss lands on the customer and on Yapily's numbers alike.
...
**What it covered.** Not just the consent screen. Guidance spanned the full set of surfaces a team building on open banking has to get right:
...
- **Consent dashboards** — where people see and manage what they've already granted
...
**The through-line.** Open banking design is mostly about moments the designer doesn't control. The user leaves for their bank's own interface and comes back, access lapses on a schedule set by regulation, and the handoffs are where people drop out. The guidance concentrated on those seams, because that is where the design work actually is.
...
## 4. New passage for the Hosted Pages case study — Layout for scale

*Suggested placement: as a design-decision section within the existing Hosted Pages case study, not in the Research practice section. It's an outcome of the practice; it reads best next to the product it changed.*
...
We prototyped three variants and tested them against both scenarios: an accordion, a dedicated results page, and a hybrid.

The hybrid won — accordion behaviour for small result sets, a dedicated page once the list grows, and a filter that surfaces branches within a parent institution. It solves the two conditions with one component rather than branching into two designs the customer has to choose between.
...
`[CONFIRM]` — "outperformed the production flow" on what measure? Prototype testing gives task success, time and preference, not conversion. Naming the measure makes this precise; leaving it open invites the assumption that it was a live conversion comparison.

---

## 5. Corrections to existing pages
...
## 6. Open items — do not build

- [ ] **"Outperformed the production flow" — on what measure?** Task success, completion time, preference? Worth naming precisely, since the sentence currently reads as though it could be a live conversion comparison.
- [ ] **Did the hybrid ship, and did it hold up in production?** A prototype winner that later validated against real traffic is a much stronger story than a prototype winner alone.
- [ ] **Check the "through-line" paragraph in Section 3.** I wrote that the guidance concentrated on the handoff moments — bank redirect, return, expiry — because that's where users drop out. That's my reading of why those surfaces were chosen, not something you said. Keep it if it's true, cut it if it isn't.
