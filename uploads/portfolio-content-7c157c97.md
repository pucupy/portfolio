...
Seven-page static site · pucupy.github.io/portfolio

Working source of truth for the site copy. Page filenames are given in each
section heading. Images are marked `[image: …]` with their placeholder or
caption; built diagrams (flow wireframes, before/after comparisons) are marked
`[diagram: …]` with their labels transcribed, since they are markup rather
than pictures.

Site navigation, contact links and "back to work" links are omitted — this is
...
**Lead Product Designer & Design Manager building regulated fintech products and design functions from zero to scale.**

As Yapily's first designer I've led product strategy and design across bank connectivity, self-service platforms, research and design systems. I manage two product designers and built the company's UX research practice from scratch.

**Evidence row**

- Founding designer at Yapily · 20+ years in digital design
- Managing two product designers · seven managed across six years
- 200+ customer interviews · hundreds of end-user usability sessions
- Regulated B2B2C fintech

**Primary actions:** View selected work · Download CV · Email me
...
**Role:** Product strategy, service mapping, information architecture · **Period:** 2024–2026 · **Status:** Beta
**Outcome:** Hosted Pages configurator shipped in full; self-serve onboarding prioritised across five functions
...
**Role:** Built all three, then handed each to a designer on the team · **Period:** 2020–2026 · **Status:** In use, maintained by the team
**Outcome:** Three systems on one token layer, with contribution governance built jointly with the frontend lead
...
**Role:** Built and ran it, with no dedicated research function · **Period:** 2020–2026 · **Status:** Running
**Outcome:** 200+ customer interviews, hundreds of usability sessions, and the experimentation pipeline the conversion numbers came from
...
**Role:** Lead Product Designer · **Period:** 2019 · **Status:** Shipped, company pivoted on the result
**Outcome:** £150,288 ARR from 117 customers in three months — four times the prior model, and a change of revenue model

HeadBox was an Airbnb for event spaces: search a directory, filter, message a few venues, wait. It converted at around 10%, and the research explained why from both sides at once.

Guests were overwhelmed rather than underserved. They aren't event experts, don't know what they want, and were handling too many options — so they'd shortlist a few venues, Google them, and phone them directly. Hosts had the opposite problem: too many leads from too many sources, almost none of them matched to their venue, so real enquiries got lost in the noise and hosts stopped logging in. A CRM built before I joined was meant to fix that and didn't; it was hard to use, and venues never signed in, so revenue leaked. Some of it leaked deliberately — passing contact details outside the platform to avoid the commission is a structural weakness of any marketplace taking a percentage.

Meanwhile 80% of the company was sales and account managers working in spreadsheets and slides. That's defensible for large corporate accounts and hopeless for the many small and medium ones, which is where most of the volume was.

The insight that broke it open was that we had the expertise on the wrong side of the transaction. Guests were being asked to make expert decisions; hosts, who are the experts, were being handed a queue they couldn't act on. So we inverted it. Guests complete a short wizard that captures requirements instead of browsing a directory. Those enquiries pool into a feed. Hosts choose the events they want rather than events choosing them.

That inversion changed the business model as a consequence rather than as a goal. Venues pay for access to the feed instead of a commission on the sale — which also removes the incentive for the off-platform leakage, because there's no percentage left to avoid. Marketplace to SaaS.

Fifteen customer interviews had already refocused the team on the enquiry-to-booking funnel, where only 20% of enquiries received a host proposal. The Enquiry Hub shipped as a deliberately small experiment — London only, capped at £10k — which showed the platform connected hosts and guests well but converted poorly, and that's what justified the larger bet.

Prototypes started as a spreadsheet of events a host could pick from, which was enough to test whether the content was sufficient and whether the idea was exciting, before anyone built anything. It evolved through Figma into coded prototypes. Over 1,500 leads in the first month, and hosts logging in daily from launch — the metric that had defeated every previous attempt.
...
- Selected illustration, identity and animation work behind the product practice.
- View the archive →
...
**Role:** Lead Product Designer & Design Manager · **Scope:** product strategy, service mapping, research, information architecture, interaction design · **Partners:** product, engineering, customer success, support, sales, operations · **Status:** beta, with the Hosted Pages configurator shipped in full

Becoming a Yapily customer took six months to a year, and almost none of that was technical. The Console couldn't do what customers needed, so the gap was filled by people: sales, pre-sales, implementation, customer success and support completing routine tasks by hand, per customer, repeatedly.

This is the work to turn that into a product. Smaller customers get a self-serve route from evaluation through sandbox testing, business verification and into production. Enterprise customers keep an assisted path — on the same platform underneath.

`[image: The lifecycle — first run, credentials, verification, analytics · 2200×1120px]`

### Six years in four moments

- **2020** — a certificate vault and application manager, built for the handful of customers who held their own licence.
- **2021** — a research-led redesign of onboarding and usability. Better tool, same shape.
- **2022** — a second licensing model and thousand-bank markets arrive. The Dashboard becomes the Console.
- **2022–2025** — rebranded repeatedly, barely extended, while engineering migrated from a monolith to microservices. The API kept moving; the Console didn't.
- **2025–2026** — the gap was wide enough to justify asking what the tool was for, rather than what to add back.

The full version is at the end, for anyone who wants it. What matters here is the consequence.

`[diagram: console-evolution-4-2026.svg — the organisation becomes the container]`

### The problem was operational, not cosmetic

**Nobody could get live without us.** Onboarding needed so much hand-holding that there was no path through it without several Yapily people being personally involved. Not one blocking step — a sequence of them, each requiring a different person.

**Everyone got the same journey.** Google and a corner shop went through an identical process. The largest customers weren't getting the attention their complexity warranted, and the smallest were sitting through a sales-led onboarding they neither needed nor wanted. Around 80% of the customer base is non-enterprise, on customer success's figures.

**Product gaps became human queues.** Nearly every stage past the contact form was annotated as manual, for staff and customers alike. CSMs exported reports by hand, support engineers registered sandbox banks, PMs enabled feature flags. None of it looked like a design problem. It looked like we needed more people.

Two more findings shaped the response. Before someone became a customer we couldn't identify them across the website, docs, dashboard or support — so nothing could be tailored, and people hit limits they had no way to know existed. And direct customers and agents have materially different needs; one agent's note in the research was simply that they didn't want to see our logo in their experience. One assisted funnel served neither well.

`[image: The service map — manual steps highlighted across five functions · 2200×1120px]`
...
I ran three things, each doing a different job.

**Customer interviews**, to establish what people were actually blocked on rather than what we assumed.

**A cross-functional workshop** with VPs, PMs, engineering, support, design and customer success — to produce one shared account of the journey rather than five departmental versions of it. I mapped the whole thing end to end beforehand: every stage from anonymous visitor to renewal, every team that touched it, every tool it ran on, and what mattered to the person at each stage, split three ways because a buyer evaluating providers, a signed-but-not-live customer and a customer in production don't want the same things. It was deliberately not a design artifact.

**A two-day hackathon**, to make the friction concrete for people who had never felt it. Engineering, PMs, design and implementation, role-playing as an outside consultancy using only what a real customer can access. It surfaced a 403 because Hosted Pages wasn't enabled by default, needing a two-day wait for a PM inside a two-day hackathon; sandbox banks registered by hand, roughly a third of the pre-configured ones broken; three returned IDs with no way to tell which was the Application User ID; and an undocumented CORS issue forcing customers to build their own backend proxy.

None of it was new to the customers experiencing it. What was new was that our own engineers and PMs had now experienced it.

Two were fixed directly off the back of it. Engineering resolved the CORS issue. And the sandbox was rebuilt to contain every authentication flow, so a customer could exercise the whole surface without asking us to enable anything — which paid a dividend nobody had scoped for, because it also gave automated testing a complete environment to run against.

The main resistance came from senior management on the platform side, and the objection was fair: the priority was launching integrations in more regions, that's the growth engine, and Console work would take capacity from it. A concrete bet against a vaguer one. The workshop is what changed it — once sales, support, customer success and engineering were in one room describing the same journey, the cost stopped being a design opinion and became a visible amount of several teams' time.

`[image: Hackathon output — the friction log · 2200×920px]`

### The strategic decision: split the path

Enterprise customers keep the assisted journey, because that's what they want and it's worth doing well. Everyone else gets a self-serve one: find the site, get into the Console, and onboard and implement using the product and the documentation without waiting on a calendar.

**Self-service wasn't the removal of support. It was the removal of support as a default dependency.**

Target: weeks to a few months from first contact to live, against six months to a year.

`[image: Before and after — the same journey, with the manual dependencies removed · 2200×1120px]`
...
A new customer gets a preconfigured sandbox from signup — free, with mock data — and a route to their first successful API call that doesn't pass through us. Production is an explicit upgrade rather than the only door, which is the direct answer to a 2021 finding that people evaluating the API got the same experience as customers running in production.

Credentials are labelled by sensitivity: application ID and client ID marked safe to share in client-side code, the secret handled differently. Secrets can be revealed rather than shown once, and regenerating one issues the new secret with a twenty-four-hour grace period on the old so nothing breaks mid-rotation.

That last detail is a 2021 finding answered properly. The audit then recorded that downloading your application secret also closed the window — a sudden, unrequested action at the most sensitive point in the flow. It was logged as a usability bug. It was always a credential-lifecycle problem.

The quickstart guide is stateful now: steps tick off as they're completed and the list persists on the home screen rather than being a welcome mat you walk over once.

`[image: First run — sandbox ready, credentials in reach · 2200×1120px]` · **Beta**

### Configure the integration

The institutions view is aimed straight at the largest item in the operational bill. *Which banks support what* had been a question people asked a human, and CSMs answered by hand, per customer, repeatedly. It's now a filter: by product, account type, payment type and payment rail; by whether a bank supports app-to-app, SEPA Instant or account-holder-name checks; with a toggle to hide banks currently having incidents.

It also switches between individual banks and bank groups — the same idea as the brand grouping in Hosted Pages, arrived at from the opposite direction. There it stopped end users drowning in near-identical branches. Here it lets an integrator reason about a banking group as one thing when that's the useful unit, and drill in when it isn't.

Hosted Pages configuration sits alongside it: four values — brand colour, text colour, button text colour, page background — set against a live preview of the customer's own consent screen. Webhooks carry the operational tooling a developer actually needs: test events, delivery logs, success rates, and replay for anything that failed.

`[image: Institutions — bank capability as a filter, not a question for a human · 2200×1120px]` · **Beta**

### Move to production

Onboarding runs as seven steps — use case, industry, plan, KYB, review, sign, complete — saveable and resumable at any point. That isn't a detail. A flow demanding a certificate of incorporation and a director's ID is not one anyone completes in a sitting, and a wizard that loses your place is a wizard people abandon.

Step one asks whether the customer holds their own open banking licence. Given what that question's absence cost twice over, asking it before anything is configured is the point.

KYB is tracked as four visible states rather than a black box — business information, company documents, authorised signatory, agreement signing — each showing not started, in progress or done. Verification carries a renewal date, because re-verification is a recurring obligation rather than a one-off. Compliance moves from a scheduled in-person session to an asynchronous queue, and that single step was the most common reason a go-live slipped, for scheduling reasons rather than substantive ones.

`[image: KYB as four tracked states, with renewal · 2200×1120px]` · **Beta**

### Operate and improve

The analytics are the part I'd point at, because of where they came from. The event instrumentation and experimentation pipeline described in the research practice were built so *we* could tell whether a change had worked. The same funnel is now the customer's: institution selected, authorisation started, authorisation completed, payment confirmed, with the drop-off between each step. Apparatus built to answer an internal question turned out to be what customers most wanted to see.

It's opinionated rather than neutral. A six per cent drop between institution selection and authorisation doesn't just appear as a number — it's flagged, with a suggestion about the redirect flow. Analytics that only display are analytics most people don't act on.

Bank performance sits alongside it, and makes visible the thing this portfolio keeps insisting on: conversion in open banking is substantially not a design variable. The spread across institutions runs from the low nineties to the low seventies. Design didn't cause that range and can't close it. Session-level monitoring completes the picture — request IDs, bank, duration, completed or failed — so a developer can debug a specific failure rather than infer it.

Customers are businesses, not individuals. Roles are scoped — owner, admin, billing, developer — so a finance person isn't navigating API keys and a developer isn't blocked waiting for access. Nor is a customer necessarily one company with one application: those operating across regions hold multiple organisations, and those reselling to their own customers create sub-applications beneath their own.

The sub-application model is the part I'd defend hardest, because one construct solves two problems. A payments provider's merchants and a customer's regional entities look different from outside and behave identically underneath: each needs its own products, its own countries, and — the part that makes it non-trivial — its own compliance state. So a sub-application carries its own KYB status. One can be approved and live in Germany while its sibling sits under review in France and a third is still pending, all beneath the same parent. Compliance isn't a property of the customer; it's a property of each entity the customer operates. Modelling it any higher would have forced the whole account to move at the speed of its slowest verification.

`[image: Conversion funnel with drop-off flagged, and per-bank performance · 2200×1120px]` · **Beta**

`[image: Sub-applications, each with its own products, markets and KYB state · 2200×1120px]` · **Beta**

### Decisions worth defending

**Why enterprise kept an assisted route.** Self-serve as a universal answer would have been cheaper to build and worse for the customers who generate most of the revenue. Complexity that genuinely warrants a solution architect should get one; the mistake was making that the default for everybody.

**Why sandbox is immediate but production stays gated.** Evaluation has no regulatory weight and should cost nothing. Production moves real money under a licence, and no amount of self-service design removes the verification underneath it. The design question was never whether to gate it, only whether the gate should be a queue or a calendar.

**Why organisation roles are separate from application configuration.** People and permissions change on a different clock from technical setup. Coupling them means a developer leaving becomes a configuration problem, and a finance person needing an invoice becomes an access request.

**Why institution intelligence is global but scope is per-application.** What a bank supports is a fact about the bank. What an application connects to is a decision about the product. Merging them would mean re-answering the same coverage question inside every application a customer owns.

### What shipped, what's in beta, what's direction

Being precise about this matters more than the screens.

**Shipped.** The Hosted Pages configurator, in full. Customers set branding across every screen in the flow themselves. Before it, a theming change meant a support ticket and an engineer, every time — a support person and an engineer consumed per request, for work that was never engineering work. Those requests stopped. That's the whole Console argument in one feature: the cost wasn't a bad screen, the cost was people.

**In beta.** The new Console — organisation and team management, application and credential management, institution intelligence, KYB, analytics. Every screen above is from that build. It's too early for adoption metrics and I'm not going to imply otherwise.

**Direction.** End-to-end self-serve onboarding as a complete path, and the full non-enterprise journey it implies. The map exists, the initiatives are scoped, and some of it isn't built.

The journey map and the information architecture are mine. The Console was built with product and engineering, and the self-serve initiatives were scoped jointly off the back of the mapping and the hackathon.

### What I'd measure next

No adoption data exists yet, so rather than reach for a number I don't have, this is what I'd instrument first:

- **Time from registration to first successful sandbox call.** The single best proxy for whether self-serve works.
- **Support contacts per onboarding stage.** The whole thesis is that these fall. If they don't, the thesis is wrong.
- **Sandbox-to-verification conversion.** Whether evaluation actually turns into intent, or the free tier just absorbs tyre-kickers.
- **Time in each KYB state.** Verification was the most common reason a go-live slipped; a queue only helps if it moves.
...
The returning-user work is the one instance where we asked in time, which is the only reason it reads as a configuration decision rather than a rebuild. And the clearest sign the lesson landed is step one of the new onboarding, asking about the licence before anything is configured. It took two expensive retrofits to learn that a question worth asking in year six was worth asking in year one.
...
`[image: The Dashboard, 2020 · 2200×1120px]` · **Superseded**

`[diagram: console-evolution-1-2020.svg — one licence model, the application as container]`

**2021 · THE FIRST REDESIGN**

The Dashboard predated me, so the first real work was establishing what was wrong with it — through fourteen interviews with engineers, usability sessions with developers, internal research with customer success and support, and analytics.

Three findings shaped the response. We weren't tailoring anything to customer type. The documentation didn't let people successfully test the API, which is the one thing an evaluating engineer is trying to do. And the dashboard didn't guide anyone through their first steps.

The audit list matters less item by item than as a pattern. Critical information lived in modals that appeared at the wrong moment and got dismissed. Password rules were revealed only after you'd broken them. Terms could be accepted but not declined. The institution list had no pagination. And downloading your application secret also closed the window.

None of it exotic — it's the accumulation you get when a tool grows without anyone owning the whole of it. The redesign went after the sequence rather than the screens: password rules stated before you type them, error messages that say how to fix the problem, registration with a choice of email or Google, and a quickstart guide answering *what do I do next* in four steps.

What it didn't change was the shape. Still a certificate vault and an application manager, made considerably better. Whether that was the right scope wasn't asked for another four years.

`[image: Before and after — the 2021 redesign · 2200×1120px]` · **Superseded**

**2022 · A SECOND LICENSING MODEL, AND GERMANY**

Two things arrived at once. Yapily Connect let customers operate under our licence instead of holding their own, creating a second customer type with different needs. And new markets — Germany above all — took coverage from a few hundred institutions to thousands. A new version shipped for both, and the Dashboard became the Console.

Scale changed what the old screens were being asked to do. A certificate list built for a handful of registrations is a different object at nineteen thousand rows.

`[diagram: console-evolution-2-2022.svg — two licensing models, one container]`

**2022–2025 · REBRANDED, NOT REBUILT**

Several rebrands and almost no new functionality. Yapily was migrating from a monolith to microservices, and Console work sat behind it each time.

The consequence was gradual and then obvious. The API kept moving; the Console didn't. New platform capability shipped with no way to reach it from the interface customers logged into, and a tool that had been adequate became one that couldn't do the job — not because anyone decided that, but because everything around it changed and it stayed still.

`[diagram: console-evolution-3-2022-2025.svg — guidance that ends at a sales form, configuration that ends at a support ticket]`

**2025 · A REASON TO RETHINK IT**
...
The architecture is visible in the shipped code, which is the version I'd rather be judged on than a Figma inventory. Yapily UI provides roughly forty shared components — button, card, dialog, select, table, menu, upload, bottom sheet, dashboard layout and the rest — and product layers sit on top of it: around thirty components specific to hosted pages, and a smaller set for the accounts app. Tokens run three deep, from global primitives to semantic roles to component-level variables, so a customer theming hosted pages overrides the component tier and never touches a component.
...
The Hosted Pages configurator is that principle made literal. A customer sets four values — brand colour, text colour, button text colour, page background — against a live preview of their own consent screen, and those four map directly onto component-tier tokens. It's the narrowest possible opening onto the system: enough for the flow to read as the customer's product, not enough for anyone to break a component. Every theming request that used to be a support ticket and an engineer is now those four fields.
...
Around 38 engineers and 4 designers build against them. That number is the point of the governance below: at that many consumers, a system only stays coherent if there's an agreed route for adding to it, and the alternative — everyone forking a component when it doesn't quite fit — is how design systems quietly stop being systems.

Three of those engineers have contributed components through the process, start to finish. That's a small number against thirty-eight, and I'd rather give it accurately than dress it up: most people consume the system rather than add to it, which is what you'd expect. What three establishes is that the route works and isn't only open to designers — the case that the governance is real rather than a document, made by people outside the team using it.
...
## Research practice (research-practice.html)
...
- brand, campaign and app
- Out-of-home campaign, Oxford Circus
...
- Product strips — Payfriendz and Monzo
