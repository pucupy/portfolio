...
**Role:** Built all three from scratch, then handed each to a designer on the team · **Period:** 2020–2026 · **Governance:** built jointly with the frontend lead · **Status:** In use, maintained by the team

Three systems — Custard, Mark and Yapily UI — one shared brand core, and a governance model that decided what got in. Around 38 engineers and 4 designers build against them.

### Why three, not one

A single system would have forced three very different surfaces into one set of compromises, and whichever surface was least like the others would have come off worst.

**Yapily UI**, internally *Pipes*, serves the console and internal tools. Dense by nature — analytics and data-heavy pages where vertical rhythm is tight and information density is the point.

**Mark** serves the marketing site. Restyled roughly annually with distinctive, playful work: larger heading spacing, more expressive layout, gradients. Coupling it to the product systems would have meant every marketing refresh rippling into production tooling.

**Custard** serves the white-label hosted consent pages. Deliberately unbranded, because a customer's brand sits on top of it.

Three separate Figma libraries on a shared token library, tokens held as Figma variables and mirrored in code, components documented in Storybook. Same primitives, different compositions — which is what made three systems cheaper than one compromised system rather than three times the work.

`[image: Three design systems on a shared token layer · 2200×1120px]`

### The sharing is asymmetrical, and that's the point

Yapily UI and Mark share a brand core: the same teal, the same ink, the same typeface, the same 4px spacing base. Change the teal and the console and the website move together. That's the whole reason to have a core.

Custard inherits none of it. It takes the ergonomics — the 4px base, the 44px minimum touch target — and nothing brand-shaped. That isn't an oversight to fix later: for a white-label product, inheriting the vendor's brand is a bug. The promise is that the customer's brand sits on top and ours isn't visible.

So Custard runs a neutral palette of its own and, tellingly, a different typeface family from the one the Yapily systems use. Not a different weight or scale — a different family. Had type been in the shared core, that alone would have made every hosted page quietly recognisable as ours.

**Shared components, separate brands**

Mechanically, Custard isn't a separate component library. It uses Yapily UI's components and overrides the brand tokens on top of them — so a button in a hosted page is the same button as a button in the console, with every value that carries brand replaced.

That's what the three-tier token structure exists to make possible. Primitives carry raw values and no meaning. Semantic aliases name a job — surface, text, action, border, focus. Components bind only to aliases, never to primitives, so swapping the alias layer re-skins an entire surface without touching a single component.

It's also what the Hosted Pages configurator sits on. The four fields a customer sets — brand colour, text colour, button text colour, page background — write into that override layer. Four values, one theming decision, no component ever edited. Before it existed, every one of those changes was a support ticket and an engineer.

### What we deliberately didn't share

The exclusions took more argument than the inclusions, and they're the part I'd defend hardest. Each one is a case where sharing was technically possible and would have made the system worse.

**The marketing lime stays in Mark.** It's the right amount of energy for a landing page and the wrong amount for something a developer stares at for six hours.

**The neutral ramp stays in Yapily UI.** Dense data needs eight greys to build hierarchy without drawing lines everywhere; a marketing page needs about three. Merge them and the website gets an unusable palette while the console gets a flat one.

**Status colours stay in Yapily UI**, because marketing has no states to report. **Gradients stay in Mark**, because console surfaces stay flat so data stays legible.

**Heading weight is shared as a token and set differently in each** — heavier and tighter on Mark, lighter in the console so headings don't shout at someone who has been reading them all day.

The rule underneath: share what's structural, keep what's tonal. Spacing, radius, motion and type scale are structural. How loud something is, is tonal — and tone is exactly what differs between selling a product and using one.

### Three decisions that kept it honest

**If a component token merely repeats an alias, delete it.** Without that rule component tokens breed: every new component arrives with a full set of its own, the alias layer stops being the source of truth, and the token file becomes a rename of your CSS rather than a model of your decisions.

**Separation by border, not by shadow.** The console leans on borders and background steps, reserving shadow for things that genuinely float — popovers, dialogs, a card lifting on hover. Five elevation steps exist and most of the interface uses none of them. Shadow on every surface in a dense view makes the page read slowly and makes the floating things indistinguishable from the static ones. Mark can afford depth because it has room; the console can't.

**Layout is a token, not a media query.** Custard renders inside a modal, an iframe or a full page, on whatever device a bank's customer happens to be holding. So modal width, content width, inset, header height, section gap and control heights all carry per-breakpoint values rather than being scattered through components.

The one I'd point at is the modal radius: 24px on desktop, 20px on tablet, zero below 600px. At phone width the card stops pretending to be a card and becomes the screen, because a rounded rectangle inset inside a viewport it already fills is just wasted edge. The touch target holds at 44px across every breakpoint — the one value that doesn't scale, because fingers don't.

### Accessibility in the tokens, not checked afterwards

Two decisions did most of the work, and both are cheap in a token layer and expensive as a retrofit.

**Status colours ship as pairs.** The saturated accent is for bars and icons; text uses a darkened partner. The saturated green on its own tint is around 4.2:1 — fine as a stripe down the side of an alert, a failure as the words inside it. Encoding that as two tokens rather than one means nobody has to remember the rule.

**The mint is a dark-mode colour only.** It fails on white, so it isn't available there. A token that only exists in the mode where it passes can't be misused in the mode where it doesn't.

The floor across all three systems: AA contrast on text, visible focus on every interactive element, 44px minimum touch target, icon-only controls carrying labels, and error fields wired with `aria-invalid` and `aria-describedby`.

### Explore the systems

Rather than screenshots, the three systems are documented here as a live reference — switch between Yapily UI, Custard and Mark, and inspect the shared core that sits under two of them.

`[embed: yapily-design-systems.html — interactive design system reference · full width, 900px tall, lazy-loaded]`

Everything in it is read from the compiled stylesheets rather than redrawn: the palettes, the semantic token table with its light and dark mappings, the type and spacing scales, elevation, motion, and the component set in both modes.

**Two things worth clicking.** The semantic token table is where the argument lives — components never reference raw hex, they read variables that remap between themes, and that single indirection is what lets one component set serve light mode, dark mode and a white-label surface. And the dark-mode strip at the end of the Yapily UI section is the same components as the light one, with nothing changed but the token values.

Alongside it, the Figma library documentation covered anatomy, states, spacing and usage rules for each component, so a decision only had to be made once.

- `[image: Bank list row anatomy — parts, dependencies and what each one inherits]` · ANATOMY
- `[image: Button footer — twelve variants of consent copy, device and region]` · COMPONENT
- `[image: Toasts — four variants, with placement rules]` · COMPONENT

The button footer is the one that justifies the documentation effort: twelve variants covering consent copy, device and region. Nobody reconstructs that from memory correctly, and without it written down every team would have guessed differently.

### Contribution and governance

The gate was evidence rather than seniority.

**Propose** — anyone could propose a component; in practice proposals came from design and frontend. A simple mockup, enough to evaluate, not a finished spec.

**Validate** — tested for usability and accessibility *before* it was eligible for review. A mix of guerrilla testing, Maze for unmoderated usability, and Testbirds for broader usability and accessibility coverage.

**Document** — the proposer wrote it up with its use cases. Defining where a component *shouldn't* be used mattered as much as where it should.
...
**Approve** — two stages: design-side first, then a group decision with the frontend team.

**Publish** — approved components entered the shared library.

`[image: Component contribution and governance flow — proposer's work in gold, collective gates in grey · 2200×840px]`
...
Shared tokens were governed more tightly than components, and correctly so — a shared-token change lands on all three systems at once. Those required approval from both the frontend lead and the design lead, and in practice happened rarely, effectively only during rebrand work.

### Where it stands

Around **38 engineers and 4 designers** build against the three systems. Three of those engineers have taken a component through the full process, start to finish.

That's a small number against thirty-eight, and I'd rather give it accurately than dress it up: most people consume a system rather than add to it, which is what you'd expect. What three establishes is that the route works and isn't only open to designers — the case that the governance is real rather than a document, made by people outside the team using it.

Each system was handed to a designer on the team once it was established. I'm currently leading the migration of the systems into AI tooling as part of a company-wide AI transformation.

### What I'd do differently

**We never formalised versioning or deprecation**, and that's the gap I'd close first. What I'd put in place now: semantic versioning, so someone has to consciously decide whether a change is breaking; deprecate rather than delete, with the replacement and migration path named; a changelog written for humans; Figma and code versioned in step so designers can tell what's actually in production; and release notes pushed to Slack rather than a wiki nobody opens.

**I'd have measured adoption from the start.** I can tell you how many people build against the systems and how many have contributed. I can't tell you what proportion of the interface is actually composed of library components versus one-off work — which is the number that would say whether the systems are doing their job, and the one I'd instrument first if I were starting again.
