...
pucupy.github.io/portfolio · eight pages plus two interactive embeds

Working source of truth for the site copy. Page filenames are in each section
heading, and pages appear in nav order. Image placeholders are marked
`[image: …]`. Navigation, footer and "back to work" chrome is omitted —
this is the content, not the shell.

Exported and cleaned 2 August 2026. Revised 4 August 2026: the 2016–2018 work
is now grouped under Su4erheroes as the employer, with NOMO, Keymono, Colly,
Caremoni and Mummybay as engagements beneath it; Payango added; Caremoni
rewritten around the service map; the creative archive reframed to hold
Nerdtees and Minimeter.

NOTE: this file and the published site have diverged. The design systems page
is longer live than it is here; the Console is shorter. Decide which is
canonical before publishing from either.
...
Six rebrands between 2020 and 2026, working with the marketing team and two external agencies. I led the yapily.com homepage redesign — three directions explored, from a problem-first layout to a full-bleed video approach — and led design on customer case studies, personalised demos and other collateral.
...
A product design consultancy set up with four former Payfriendz colleagues. Two products of our own, one principal client, and a handful of engagements alongside. I led design across all of it, and was a company director from January 2018.

The entries below are the work, not the company.

`[image: Su4erheroes — the four engagements · 2200×840px]`

### NOMO

Docomo Digital · 2016–2018 · the principal client

From January 2018, PSD2 reclassified marketplaces that hold buyers' money on the way to sellers: handling funds that aren't yours makes you an intermediary of payments. The licence costs upwards of €200,000 and takes up to 18 months to be granted. NOMO existed so a marketplace could stay a marketplace — payouts, escrow, KYC and AML handled by a licensed partner, through an API, without changing payment provider.
...
The seller app answered something the web dashboard structurally couldn't: a seller sells on more than one marketplace, and nobody was giving them a single view of what they'd earned. Payments showed a status timeline rather than a status word — pending on the 13th, paid out on the 15th — and each currency's account fields used its own format: sort code for sterling, IBAN for euro, routing number for dollar.

Also in this engagement: a consumer cashback app for Huawei customers, launched, and payment widgets for alternative payment methods and for ICO investment.

`[image: NOMO — partner onboarding, two steps · 2200×952px]`

`[image: NOMO — the seller app, earnings across marketplaces · 2200×952px]`

### Keymono

2016–2018 · our own product

Collecting money from a group, which is a distribution problem before it's a payments problem. We built the payments infrastructure underneath it as well — keymono.io — rather than renting someone else's.

The collection types are categorised by what the organiser doesn't know. *Open* — best if you're not sure how much you want to collect in total. *Split* — you know the total and the people. *Fixed* — you know the total but not how many will chip in. Each carries a concrete example rather than an abstraction. Most products would have shipped "Fundraiser / Split bill / Group gift" and made people map their situation onto a label; this asks the only question that changes the maths.
...
`[image: Keymono — choosing a collection type · 2200×840px]`

### Colly

2016–2018 · our own product

A prepaid wallet for sending and receiving bitcoin between people.

The design position was that nobody thinks in bitcoin. Balances, transactions and the amount you type are all denominated in the user's own currency, with the BTC figure underneath in quieter type. It's the same value shown twice, but the order tells you which one the product thinks you're holding.

Sending goes through contacts rather than addresses. The flow opens on your phonebook, split into people already on Colly and people to invite, so the raw address only appears where it has to — receiving and withdrawing, behind a QR code and a Copy button. Withdrawals show the network fee above the button rather than after it, and payments confirm by fingerprint. Each contact has a page rather than a log: total sent, total received, number of payments.

It reached pilot.

`[image: Colly — activity, denominated in the user's own currency · 2200×840px]`
...
The chain ran on four different clocks. Commissioning groups paid quarterly. The operator paid customers monthly. Care agencies invoiced weekly. Relatives sent receipts, invoices and bank statements quarterly, and the operator validated those against the support plan and audited back to the commissioning group on a spreadsheet, by email. Money was held back pending that audit, so customers chased for it. A new bank account was opened for every customer.
...
What was built: an operator dashboard for verifying carers and agencies, allocating budget, monitoring spend and reporting; and an app for carers and customers to request and authorise payments, and rate the service privately. The rating is confidential by design and says so on the screen — a customer who fears their carer will see it won't answer honestly. The reporting was designed to replace the emailed spreadsheet, so commissioning groups could see payments directly rather than waiting for a quarterly audit. I left before the pilot ran far enough for me to know whether they did.
...
`[image: caremoni-payment-chain.svg — before: four clocks, one reconciliation · 2200×1150px]`

`[image: Caremoni — operator dashboard and the carer payment app · 2200×840px]`

### Mummybay

2017 · UK and Singapore

Babies grow fast, and parents end up with piles of clothes they never used — often the wrong size, because people buying for a baby guess. A founder brought us an app built at a hackathon: enter your baby's age and size, it predicts the sizes to come, share the result to Facebook.

I reviewed the app, then went to talk to parents, and ran usability sessions on the existing build. The findings didn't improve the app, they ended it. Parents didn't want any of this on Facebook. Sizing isn't consistent between brands, so the prediction couldn't be right for everyone. They installed baby apps constantly and paid for none of them. And the problem they actually had wasn't predicting sizes — it was the cupboard of clothes already outgrown, which they'd tried and failed to sell because, as one put it, the existing sites weren't made for it.

What they described was a marketplace. Rather than build one to find out, I stood up a working store on an off-the-shelf platform and put it in front of the same parents. They listed items and sold them. That was the evidence for building properly: a customised version followed, with payments and a reworked interface.

Mummybay now runs in the UK and Singapore.

`[image: Mummybay — the marketplace · 2200×840px]`
...
A prepaid card product, customer and admin side. Worked remotely from Philadelphia.
...
`[image: Payango — top-up methods, fees against settlement times · 2200×840px]`
...
Illustration, identity and animation made alongside the product work — much of it drawn by hand, for fun, in the pre-AI era.

Including Nerdtees, a 2018 side project honouring scientists who don't get put on t-shirts. One system, three portraits: a circular badge, two flat colours, and the person's own discovery as the graphic device — lightning for Tesla, the diffraction pattern for Franklin, punched cards for Lovelace. And Minimeter, a baby growth tracker that reached pilot, whose name, icon, tagline and illustration were built around a single idea: a measuring instrument, and a baby too big for the building.
...
I lead design across Yapily's open banking platform. Hosted Pages is the flagship: a pre-built consent and bank-connection flow, white-labelled and compliant out of the box, that customers embed in their own product rather than building one bank at a time. Then the console they use to configure and manage their integration, and the design systems underneath both.
...
Since then I've worked on peer-to-peer payments across the UK and Germany, marketplace payouts across Europe and Asia, personal health budget payments with the NHS, and a venue-booking marketplace in London. Between Payfriendz and HeadBox I ran a product design consultancy with former Payfriendz colleagues — mostly marketplace payouts for Docomo Digital, alongside two products of our own.
...
Two different counts appear in this case study and they measure different things. Forty-plus is the number of *flow configurations* — distinct paths a real user can take through the product, determined by that branching. The hundreds referred to later are *experiment variants*: copy and layout versions tested against production traffic between 2023 and 2026. One is the shape of the product; the other is how many times we changed it.
...
*01 Merchant checkout The customer’s own page. Hosted Pages has not opened yet — everything after this is decided by how they integrated.*
...
Commerzbank Used 2 hours ago

RECENT

Sparkassen Regional banks

Volksbanken Raiffeisenbanken 700+ local banks

OTHER BANKS A–Z

ABN AMRO

*02 Choose your bank Hosted Pages opens over the merchant page. Search, popular banks and recents — the screen most of the research went into.*
...
Scan the code to approve €48.00 with Commerzbank on your phone.
...
Tap the link Opens the Commerzbank app, or its mobile site

3

Approve the payment We’ll pick up here
...
*03 Handoff to the bank app Desktop only. After bank selection and the consent step, the user lands either here or straight on the embedded login, depending on what the bank supports. Scanning opens the bank’s app if it’s installed and its mobile site if it isn’t. There is no QR screen on mobile — the code exists only to move someone from desktop to phone. Removed entirely in markets where app-to-app was thin.*
...
PIN Never enter a one-time code here.
...
*04 Authenticate Bank redirect, or embedded login — the bank named, the regulated entity stated, and the fields mapped to what that bank actually asks for.*
...
Payment approved Returning you to ACME

£248.00 · Commerzbank Reference 4419-KD

*05 Back to the merchant Consent granted and control handed back — the point the whole flow is measured against.*
...
`[image: Seven of the twenty-one hosted screens — bank selection, embedded login, a declined payment, the QR handoff, the account-access consent, and two mobile screens]`

*Twenty-one hosted screens — payment, account access, the mobile variants and the failure states the happy path hides.*
...
*BEFORE A flat alphabetical list of every institution*

What replaced it is in the flow above — bank selection and a brand group opened ↗ .
...
VARIANT 2 Two-column grid and list
...
The selector sits in the header of every screen in the flow above, on both desktop and mobile — see it on the mobile screens ↗ .
...
*BEFORE A login form with nothing to place it*

What it became is in the flow above — see the embedded login ↗ .
...
Bank discoverability — time to find a bank fell from 5–15 seconds to 1–3 in most markets and 3–5 in branch-heavy ones, through display naming matched to what people call their bank, search alias gaps found in testing and passed to engineering, and the grouping and shortcut redesign of bank selection.

Embedded login — roughly 13%, across field mapping, A/B tested trust signals and the per-bank retry corrections.

The decoupled handoff — roughly 10% in the regions where QR was retained, plus an improvement in the regions where the step was removed entirely.
...
A certificate vault and application manager, built for the handful of customers who held their own licence.

2021

A research-led redesign of onboarding and usability. Better tool, same shape.

2022

A second licensing model and thousand-bank markets arrive. The Dashboard becomes the Console.

2022–2025

Rebranded repeatedly, barely extended, while engineering migrated from a monolith to microservices. The API kept moving; the Console didn’t.

2025–2026

The gap was wide enough to justify asking what the tool was for, rather than what to add back.

The full version is at the end, for anyone who wants it. What matters here is the consequence.

`[image: The Console, 2026 beta — the organisation becomes the container]`

*Where it landed. Teams, billing, KYB and analytics sit at organisation level; applications hold sub-applications, each carrying its own compliance state. Product scopes are gated by KYB and billing rather than by a support ticket.*
...
The same product once the customer is live. The apparatus here — funnels, drop-off, per-bank performance, session monitoring — was built so we could tell whether a change had worked. It turned out to be what customers most wanted to see.
...
It wasn't called the Console. The first version was the Dashboard, built for a handful of customers who held their own open banking licence — connecting them to our API and a few hundred banks. Functionally it was two things: a certificate vault, and application management. That was the whole scope, and at the size the company was then it was the right one. Yapily had five customers in 2020, Intuit among them.

What it couldn't tell a customer was anything about how their integration was performing. No API performance, nothing they could act on without asking us. Hosted Pages didn't exist yet as a concept, and neither did Yapily Connect.

`[image: The Dashboard, 2020 — one licence model, the application as container]`
...
`[image: The Dashboard, 2020 — applications, certificates and links out to documentation · 2200×1120px]`
...
`[image: The Dashboard before — welcome screen with no next step · 2200×1120px]`

*BEFORE A welcome screen with no next step*

`[image: After — quickstart guide, and password rules stated before you type · 2200×1120px]`

*AFTER Quickstart guide, and password rules stated before you type*
...
Two things arrived at once. Yapily Connect let customers operate under our licence instead of holding their own, which created a second customer type with different needs. And new markets — Germany above all — took coverage from a few hundred institutions to thousands. A new version launched to support both, and the Dashboard became the Console.

Scale changed what the old screens were being asked to do. A certificate list built for a handful of registrations is a different object at nineteen thousand rows, and the work here was as much about making volume navigable as about adding features.

`[image: The Console, 2022 — two licensing models, one container]`
...
`[image: Console wireframe — certificate management at scale · 2200×1120px]`
...
`[image: The Console, 2022 to 2025 — guidance that ends at a sales form]`
...
`[image: Console 2022 — application settings, with the layout under strain · 2200×1120px]`
...
## Case study 03 — HeadBox (headbox.html)
...
Role: Lead Product Designer · Period: 2019 · Company: HeadBox, enterprise event technology

Status: Shipped; the company pivoted its business model on the result

Outcome: £150,288 ARR from 117 customers in three months, four times the prior model · 1,500+ leads in the first month · hosts logging in daily from launch
...
*SUPERSEDED The homepage in 2019. A search box that assumes expertise, and a phone number for when it doesn’t.*
...
*BEFORE Listing a space in 2019: prose-first, directory-shaped, and no sense of whether what you’d entered was enough to be matched on.*
...
← All work · Research practice →

← All work · aleuxuk@gmail.com · LinkedIn
...
## Case study 04 — Design systems (design-systems.html)
...
`[image: The design system reference — the Button section, with live variants and their token specs]`
...
## Case study 05 — Research practice (research-practice.html)
...
## Embed — Design system reference (design-system-reference.html)

Yapily UI v3 · reference

Yapily UI Custard Mark
...
Colour Typography Spacing and radius Elevation Motion
...
Text input Search Select Checkbox and radio Switch

Display

Badge and tag Avatar Stat Table Skeleton and spinner
...
Bank list Consent summary Credentials panel

Custard

Theme contract Page shell Bank selection Consent Status screens Button footer

Mark

Colour and gradient Display type Pill CTA Hero Cards and media Stat band
...
The saturated value is the accent — bars, icons and borders only. Text uses the darkened partner. Measured ratios below.

Success

#08611F · 7.2:1 #09893C · 4.2:1 non-text

Info

#1C398E · 9.5:1 #1447E6 · 6.3:1 non-text

Warning

#973C00 · 6.9:1 #D08700 · 2.8:1 non-text

Destructive

#C10007 · 5.9:1 #E7000B · 4.4:1 non-text
...
Body md — the default reading size for product copy. 16 / 26 / 400

Body sm — secondary copy and helper text. 14 / 22 / 400
...
Primary Secondary Ghost Delete application Disabled Saving

height 44 / 36 / 52 radius 8 padding-x 20 label 15 / 500 transition 150ms

### Icon button

Square, 44px, and always carries an accessible name — the glyph is never the label.

⧉ ↻ ⋯ ✕

44 × 44 radius 8 aria-label required

### Button group

Segmented selection where the options are mutually exclusive and few. More than four options is a select.

Sandbox Production

role="group" selected: primary fill arrow keys move selection

### Text input

Label above, helper below, error replacing helper rather than joining it. Errors say what happened and what to do next.

Application name Shown to your team, not to end users. Redirect URL Not a valid URL. Include the scheme, for example https://. Organisation ID Set at creation and cannot change.

height 44 radius 8 error: aria-invalid + describedby
...
type="search" aria-live="polite" count debounce 200ms

### Select

Native element, styled. A custom listbox is only worth its accessibility cost when options need more than a label.

Country United Kingdom Germany France Ireland Product scope Account information Payment initiation Both

native select height 44 label always visible

### Checkbox and radio

Checkbox for independent options, radio for one of a set. The whole row is the target, not just the control.

Scopes Accounts Balances and account details Transactions Up to 24 months of history Payments Requires verified business

Licence model Yapily licence We hold the regulatory permission Own licence You register certificates and banks

row height 44 control 18 label wraps the input

### Switch

For settings that apply immediately. If the change needs saving, it is a checkbox with a save action.

Send webhook on payment status change

role="switch" track 44 × 26 applies immediately

### Badge and tag

Badges carry state and always pair colour with a word. Tags carry metadata and are neutral.

Live In review Failed Sandbox AIS United Kingdom SEPA Instant

height 26 badge: pill + dot tag: radius 6, neutral never colour alone
...
28 / 36 / 44 people: circle institutions: radius 10
...
value 30 / 600 tabular-nums delta names the period
...
row 48 numeric: right + tabular <700px: stacked rows
...
Verifying business · step 3 of 4

role="status" shimmer honours reduced-motion progress names the step
...
Production access is enabled. Your next re-verification is due 14 March 2027.

i

Sandbox scopes are read-only

Every scope is available for testing. Choose the ones you need when you move to production.

!

Revolut is reporting degraded authorisation

Success has fallen to 74% over the last hour. Users may need to retry. No action is required from you.

✕

Webhook delivery failed 12 times

Your endpoint returned 500 for every attempt since 09:14. Check the endpoint, then replay the failed events.

role status / alert icon + colour + word 3px accent rule
...
✓ Client secret regenerated Copy

✓ Invitation sent to rk@northwind.co Undo

bottom-centre dismiss 5s, pauses on hover one action maximum
...
Once your integration creates its first consent, every authorisation attempt appears here with its bank, duration and outcome.

Open the sandbox app Read the quickstart

title + explanation + action max 46ch never an illustration alone

### Tabs

Peer views of one object. If the panels are separate destinations with their own URLs, it is navigation, not tabs.

Overview Scopes Webhooks

{{ tabBody }}

role tablist / tab / tabpanel arrow keys move 2px underline on selected

### Breadcrumbs

Where the current page sits, and a route back out. The current page is present but not a link.

Applications / Payments service / Webhooks

aria-label="Breadcrumb" aria-current="page" separator hidden from AT

### Pagination

Range and total stated, because a page number alone does not say how much is left.

1–25 of 412 sessions

← 1 2 3 … 17 →

44 × 44 targets aria-current="page" range and total always shown
...
state: done / current / pending resumable tick is not the only signal

Patterns

### Bank list

The most-used surface in the product. Rows are 64px, the whole row is the target, and an unavailable bank explains itself rather than disappearing.

Popular banks

Barclays Lloyds Bank HSBC 3 accounts available

Revolut Temporarily unavailable — try again shortly

row 64 logo 36, radius 8 selected: brand tint + radio outage stated, not hidden

### Consent summary

What is being shared, with whom, for how long, and how to stop it — before the button, not after it.

Confirm what you are sharing

Northwind will receive the following from your Barclays account. Read-only — nothing can be moved.
...
Continue to Barclays

You can revoke access at any time from your bank or from Northwind.

definition rows expiry always stated revocation before the CTA
...
Application ID Safe in client code

app_7f2c91b8e4a03d5f
...
Regenerating issues a new secret immediately. The previous one keeps working for 24 hours so a deployment can catch up.

sensitivity badge required secret masked by default destructive action explains the grace period

### Theme contract

Custard is the white-label consent journey. A customer supplies four values and a logo; everything else — spacing, type scale, structure, the 44px targets — is fixed. Widening that contract is how a white-label system stops being one.

Token Applies to Default

--brand Primary button, selected row, links, focus ring #111827

--brand-contrast Label colour on the primary button #FFFFFF

--surface Page background behind the card #F5F6F8

--radius Card, button, input and logo tile 12px

--logo Header mark, 32px tall, any aspect SVG or PNG

Preview brand Neutral Northwind Verdant Bank Sunhaus Every example below re-themes. No component changes. Brands are invented.

### Page shell

One card, centred, 460px maximum. The customer's mark sits top-left and the security note bottom — a consent screen has to say whose journey it is and who is operating it.
...
Securely and in read-only mode. It takes about a minute.

🔒 Regulated open banking. Your credentials are entered on your bank's own site.

card max 460 radius 12 logo 32 cancel always reachable

### Bank selection

Popular banks first, then search. Selection tints with the brand rather than a fixed blue, which is why the tint is derived from the theme value instead of stored separately.

Choose your bank

Popular banks

Barclays Lloyds Bank HSBC Personal and business

row 64 selected tint derived from --brand search above 8 results

### Consent

The regulated screen. Copy varies by market and product, so the layout is fixed and the strings are data — which is what makes twelve footer variants tractable rather than twelve designs.

→

{{ cBrandName }} would like to access your Barclays account

Read-only. Nothing can be moved and no payment can be made with this permission.
...
Continue to Barclays Cancel

You can withdraw access at any time. Terms apply .

primary 52 expiry mandatory copy is data, layout is fixed

### Status screens

Success, pending and failure share one layout so a customer theming the journey gets all three for free. Failure never blames the user and always offers the next move.

✓ Account connected Returning you to {{ cBrandName }}.

Waiting for your bank This can take up to a minute. Do not close this window.

! Barclays did not respond Nothing was shared. You can try again or pick another bank. Try again

one layout, three states failure states what was not shared always a next action
...
Cancel Authorise payment

mobile: stacked, full width desktop: inline, right label from product × region

### Colour and gradient

Mark is the marketing site. It runs dark by default — deep teal ground, mint as the single accent — and inverts to light for long-form pages. Gradients are used on display type only, never on a surface.

--ground #002323

--ground-raised #003839

--accent #63DFB3 · 11.1:1 on ground

--ground-light #F4FBF9

linear-gradient(96deg, #FFFFFF 18%, #63DFB3 92%) + background-clip: text Angle and stops reconstructed from the rendered headings; the compiled rule was not readable.

one accent gradient on text only dark is the default

### Display type

Marketing headings run larger and tighter than product ones, and clamp rather than step at breakpoints. The gradient treatment is reserved for the single hero line on a page.

Payments, without the plumbing hero · clamp(32,5vw,56) / 600 / −4%

One integration, 2,400 banks section · 30 / 600 / −3%

Marketing body sits at 72% white on the dark ground — enough separation from headings without dropping below contrast. body · 17 / 28 / 400 / 72% white

clamp, not breakpoints one gradient line per page body 72% white

### Pill CTA

Mark's buttons are the pill step of the shared radius scale — the same component as the product button, at the top of the same ramp, rather than a separate one.

Book a demo Read the docs See coverage →

Book a demo Read the docs

height 52 radius pill accent fill on dark, teal on light

### Hero

Eyebrow, gradient line, one paragraph, two actions, and a media well. Composed from the parts above — the hero is an arrangement, not a bespoke block.

Open banking infrastructure Connect to 2,400 banks with one API

Account data and payments across the UK and Europe, under our licence or your own.

Start building Talk to sales

MEDIA WELL · 16:9

two columns ≥900px media 16:9 one primary action

### Cards and media

One card, three uses — feature, resource and customer story. The media well is a token-sized slot rather than a per-page crop, which is what stopped marketing pages drifting apart.

Coverage Bank capability, filterable by product, rail and market.

MEDIA WELL

GUIDE Choosing a licence model

“We replaced four bank integrations with one.”

Head of Payments, Northwind

radius 14 media 16:9 slot quote card: accent border

### Stat band

The same stat component as the product, re-skinned for the dark ground. Marketing figures carry a source line, because an unsourced number on a marketing page is a claim.

2,400+ banks across 19 markets Coverage index, Q2 2026

99.95% API availability Trailing 12 months

42s median authorisation All markets, Q2 2026

value 40 / 600 / accent source line required same component as product stat
...
## Embed — Custard payment journey (custard-payment-journey.html)

Custard — hosted payment and account access 21 screens

Payment Failure states Account access Mobile

Reference build

### The flow the customer never sees themselves building

Twenty-one screens across payment and account access, desktop and mobile, including the failure states the happy path hides. The page is deliberately close to unbranded: it takes the merchant's logo and colour and otherwise gets out of the way, because it sits between a checkout the user recognises and a bank they trust, and looking like neither is safer than looking like the wrong one.

Merchant, banks and amounts are invented. Screens are the reference build, not production captures.

“Secure page by Yapily” — the one place our name has to appear

The user is handing credentials to a page that isn’t their bank’s and isn’t the merchant’s, and regulation requires the party in the middle to identify itself. Everything else on these screens is the customer’s brand; this line isn’t ours to remove.

Payment · desktop Eight screens, checkout to confirmation

`[image: Bank selection carrying the merchant's logo and name in the header]`

Bank selection, merchant-branded

Logo, name and accent colour are the whole theming surface — four values a customer sets in the Console, with a live preview, rather than a ticket and an engineer.

`[image: The same screen with no merchant branding applied]`

The same screen, unthemed

The default has to be legible on its own, because plenty of integrations never set a logo. Recents and popular banks sit above the alphabetical list; search is the primary route and it is the first thing under the heading.

`[image: A parent institution opened to show its member banks with their own search]`

Inside a brand group

German co-operative and savings networks are hundreds of separately-connectable institutions under one name. Flattening them into the main list buries every other bank; grouping them turns one entry into a searchable set.

`[image: Review payment dialog showing total, merchant and bank before continuing]`

Review payment

Amount, payee and paying bank in that order, with the authorisation wording naming the payment provider rather than us. Cancel is a peer of continue, not a corner escape.

`[image: Embedded bank login with a warning never to enter a TAN or one-time code]`

THE HARD ONE Embedded login

The screen has to teach the user to distrust screens like this one. Credentials entered on a page that is neither the bank nor the merchant is structurally what a phishing attack looks like, so the page states who it is, what happens next, and — in the field where the harm would occur — that the bank will only ever ask for a TAN inside its own app.

`[image: Verification screen offering a QR handoff to the phone, with three numbered steps, or continuing on the same device]`

Verify — the handoff

The decoupled flow starts on desktop, moves to the bank’s app and comes back. A code with an instruction to scan it isn’t enough: the screen names all three steps, says the page will still be here, and keeps the same-device route visible for anyone who doesn’t want their phone involved.

`[image: Waiting state while the bank confirms]`

Waiting for the bank

The wait belongs to someone else’s system and can run to minutes. Naming the bank we are waiting on, and saying not to close the window, is most of what stops a user abandoning here.

`[image: Payment initiated confirmation with amount, payee and source account]`

Payment initiated

Initiated, not completed — the wording matters, because settlement is the bank’s to confirm. The return to the merchant is automatic and also manual, since an auto-redirect that fires while someone is still reading is a failure of a different kind.

Failure states Five ways the flow doesn’t finish

Most of the conversion work lives here rather than on the happy path. Every one of these states names what happened, says whether money moved, and offers a route that isn’t “start again”.

`[image: Search returning no bank, with recovery actions]`

The bank isn’t there

Coverage is never complete, so the empty result says why it might be missing, offers the full legal name as a second attempt, and gives an exit that returns the user to the merchant rather than stranding them.

`[image: Login rejected, showing remaining attempts before the bank locks online banking]`

Credentials rejected

The attempt count is the important part. A third failure locks the user out of their own online banking, which is a consequence far larger than the abandoned payment, and it isn’t ours to hide.

`[image: Payment declined by the bank, with a reference and next steps]`

Declined by the bank

We rarely get a usable reason from the bank, so the screen doesn’t invent one. It confirms no money left the account, hands over a reference the bank will recognise, and offers a different bank as the practical next move.

`[image: Timeout state after the bank sent no confirmation]`

No response

The genuinely ambiguous one: silence from the bank doesn’t mean failure. Retrying could double-pay, so the screen says to check the account first — the only case in the set where the recommended action is not the primary button.

Account access The same shell, a different consent

`[image: Consent dialog listing the data categories being shared and the access duration]`

What is being shared

Categories rather than a scope string, an explicit duration, and revocation stated before the button rather than after it. Access is ongoing in a way a single payment isn’t, so the expiry is part of the consent, not the small print.

`[image: Bank connected confirmation naming the account and the expiry date]`

Connected, and until when

The confirmation repeats the expiry date and where to withdraw access. Ninety days later this is the only screen the user will have seen that told them the connection had an end.

Mobile Where the QR code stops making sense

On a phone there is no second device to hand off to, so the handoff becomes an app switch and the QR screen is replaced rather than resized. The last screen exists only because the journey can end on a device the user didn’t start on.

`[image: Bank selection on mobile with merchant branding]`

Bank selection, branded

Rows stay at 64px and the search field keeps its position, so the same instructions work on both.

`[image: Bank selection on mobile without branding]`

Unthemed

The language selector holds its place in the header — research found language can’t be inferred from country, and the person who set the integration up isn’t the person reading this.

`[image: Review payment sheet on mobile]`

Review payment

The list stays visible behind the sheet, so the choice just made is still in view while it is confirmed.

`[image: Embedded login on mobile with the TAN warning above the PIN field]`

Embedded login

The TAN warning shortens but does not move. It sits above the field it protects, where the keyboard can’t hide it.

`[image: Mobile verification offering to open the banking app, with a browser fallback]`

NOT A QR CODE Verify

An app switch, two steps, and a browser fallback for the sizeable group who don’t have the bank’s app installed.

`[image: Screen telling the user they are done on the phone and the flow continues on the first device]`

Done on this device

The screen a scanned QR code lands on. Without it the user is left on a phone with an approved payment and no idea whether anything finished.

`[image: Payment initiated confirmation on mobile]`

Payment initiated

Same wording as desktop, same manual return. Parity here is what lets support answer a question without asking which device someone was on.

Reference build · Custard hosted pages · merchant, banks and amounts invented
